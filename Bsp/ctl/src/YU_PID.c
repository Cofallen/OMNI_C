...
uint8_t PID_F_VISION_YAW(TYPEDEF_MOTOR *MOTOR)
{
    ...
    // MOTOR[MOTOR_D_GIMBAL_PIT].DATA.AIM += TOP.pitch[5] * 0.1f;
      // 
     VISION_V_DATA.VISION_V_YAWERROR =  VISION_V_DATA.RECEIVE.YAW_DATA - (-TOP.yaw[5]);
     

	if (VISION_V_DATA.RECEIVE.YAW_DATA - (-TOP.yaw[5] )>= 180.0f)
    {
    //    VISION_V_DATA.VISION_V_YAWERROR = VISION_V_DATA.RECEIVE.YAW_DATA - (-TOP.yaw[5] + 360.0f);
       VISION_V_DATA.VISION_V_YAWERROR -=360.0f;
    }
    
    else if (VISION_V_DATA.RECEIVE.YAW_DATA - (-TOP.yaw[5] ) < -180.0f)
    {
        //  VISION_V_DATA.VISION_V_YAWERROR = VISION_V_DATA.RECEIVE.YAW_DATA - (-TOP.yaw[5] - 360.0f);
       VISION_V_DATA.VISION_V_YAWERROR +=360.0f;
    }
        //   MOTOR->DATA.AIM = ((VISION_V_DATA.VISION_V_YAWERROR) * 22.75555f);
    MOTOR->DATA.AIM = (-TOP.yaw[5] * 22.75555f) + (VISION_V_DATA.VISION_V_YAWERROR * 22.75555f);
